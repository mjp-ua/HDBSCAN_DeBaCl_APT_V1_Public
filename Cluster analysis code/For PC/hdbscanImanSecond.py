import sys, hdbscan, numpy as np, matplotlib.pyplot as plt, seaborn as sns
X = np.genfromtxt(r'TempHDBSCANfile.txt')
Y = np.genfromtxt(r'TempHDBSCANparameters.txt')
MinClusterSize = int(Y[0])
MinSamples = int(Y[1])
clusterer = hdbscan.HDBSCAN(min_cluster_size=MinClusterSize, min_samples=MinSamples, cluster_selection_method='eom', approx_min_span_tree=False).fit(X)
